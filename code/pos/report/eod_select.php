...
# http://192.168.1.66/pos/report/eod_select.php?session=207&EODID=123&date=15/06/2017&action=read&actionfinal=print

include '../config.php';
include '../functions/auth_func.php';
include '/var/www/pos/functions/print_func.php';
ob_start();

$till_session=$_REQUEST['session'];
...
if ($action=="read")
{
	...
	$results=$db_conn->query($sql_query);
	$result=mysqli_fetch_array($results);
	echo "<tr><td class=left>Till: ".$result['nicename']."</td><td colspan=2 align=right>Date: ".$_REQUEST['date']."</td></tr>";
...
where till_sessions.company=$company and till_sessions.session_number = $till_session and tills.tillname = till_sessions.till group by 1,2,3";
...
while ($onetill=mysqli_fetch_array($tills))
{
	...
	union select 'Returns' type , st.vatkey vat,sum(if(od.zero_price=1,0,if (abs(od.actualgrand)>0, od.actualgrand*abs(od.qty), od.grandTot*abs(od.qty)))) total
            ...
            and od.timestamp >= '$till_date' and od.timestamp < date_add(date_add('$till_date', interval 1 day), interval 6 hour)
            group by 1,2";
	$results=$db_conn->query($sql_query);
	unset($summary);
	$summary=array();
...
group by 1";
	...
	$pettycash=getPettyCash('192.168.1.2', $_REQUEST['EODID']);
