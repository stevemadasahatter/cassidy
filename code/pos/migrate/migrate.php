...
echo "--> Inserting rows\n";
$insert_query="insert into brands (brand, nicename) select Code, Name from test.UserDefined2";
...
echo "--> Inserting rows\n";
$insert_query="insert into seasons (season, nicename) select Code, Name from test.UserDefined1";
$do_it=$db_conn->query($insert_query);
echo mysqli_error($db_conn);
...
where BaseStyle.UserDefined2 = brands.brand
and BaseStyle.UserDefined1 = seasons.season
...
echo "--> Inserting rows\n";
$insert_query="insert into tenders (company, transno, transDate, PayMethod, PayType, PayValue, spendpot) select 1, tr.TransactionNo
,date_format(concat(substr(tr.TransDate, 1,10), ' ',substr(lpad(tr.TransTime,4,'0'),1,2),':',substr(lpad(tr.TransTime,4,'0'),1,2)),'%Y-%m-%d %H:%i')
, tr.PayMethod, tr.PayType
