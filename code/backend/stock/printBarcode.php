...
elseif ($sku<>"" && $action=="")
{
    ...
    <button onclick=\"javascript:printBarcode2('".$sku."');\">Print</button>
    <button onclick=\"javascript:closeDialog();\">Close</button></span></p>";
}
	...
	while ($result=mysqli_fetch_array($results))
	{
		for ($i=1;$i<=21;$i++)
		{
			if ($_REQUEST[$result['colour']."-".$i]<>"")
			{
				...
				for ($u=0;$u<$_REQUEST[$result['colour']."-".$i];$u++)
				{
...
function printBarcode2(sku)
{
	...
	$('#dialog').load('./stock/printBarcode.php?'+getString);
}

function populateStock(sku,colour)
{
	$('#'+colour).css('color','#ff0000');
	$('#stock').load('./stock/printBarcode.php?action=stock&sku='+sku+'&colour='+colour);
}
...
function select2(value)
{
	...
	$('#temp').show();
	$('#temp').load('./stock/printBarcode.php?action=&sku='+search2);
}
...
function searchItem2()
{
	...
	{
		$('#searchresultssc').load('./stock/printBarcode.php?action=results&brand='+brand+'&season='+season+'&sku='+sku+'&colour='+colour+'&category='+category+'&pricefrom='+pricefrom+'&priceto='+priceto);
	}
}
</Script>
