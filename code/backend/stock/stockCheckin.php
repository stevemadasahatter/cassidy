...
if ($action<>"accept" && $action<>"unaccept" && $action<>"clear")
{
	...
	echo "<tr><td><select onchange=\"javascript:select(this.value);\" name=brand>$brands</select></td>
			<td>Del Note</td><td><input onchange=\"javascript:changed();\" type=text id=note>".$_REQUEST['note']."</select></td>
			<td>Effective Del Date (YYYY/MM/DD)</td><td><input type=date onchange=\"javascript:changed();\" id=deldate>".$_REQUEST['deldate']."</select>
            <input type=hidden name=setval value=0 /><input type=hidden id=predelnote value=0 /></td></tr>";
	echo "</table>";
...
if ($action=="select")
{
	...
	while ($result=mysqli_fetch_array($results))
	{
		...
		for ($i=1;$i<=16;$i++)
		{
			$sizehtml.="<li class=\"importtitle\" align=center>".$sizearray['size'.$i]."</li>";
			$stockhtml.="<li class='importitem'><input align=center style=\"width:40px;text-align:center;\" type=text name=\"physical".$i."\" value=".$result['purchased'.$i]."></input></li>";
...
//		echo "<ul style=\"display:table-row;width:80%;\"><li class=status></li><li class=\"importitem name\" name=sku>".$result['Stockref']."</li>
//				<li name=colour class=\"importitem colour\">".$result['colour']."</li>";
		echo "<ul style=\"display:table-row;width:80%'\"><li class=status></li><li class=\"importitem name\" name=sku>";
			...
			echo "<tr><td colspan=2 class=\"importitem name\">".$result['description']."</td></tr>";
			echo "<tr><td class=\"importitem name\">".$result['costprice']."/".$result['retailprice']."</td></tr>";
			echo "</table>";
		echo "</li>";
		echo "<li class=\"importitem category\">".$result['nicename']."$stockhtml</li>";
