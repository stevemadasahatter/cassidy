...
, tilldrawer.closeval "Float" 
, if(abs(tilldrawer.startval + cash2.cash_value + cash.petty - td.float_close)>0,(tilldrawer.startval + cash2.cash_value + cash.petty - td.float_close),0) "Banked" 
...
from orderdetail od where 1=1 and od.status not in ('A','X','P','N','S') 
...
from tenders t where 1=1 and t.PayMethod in (3,4,6,7) and t.transDate [[DDATE]] 
group by 1 
union
select date_format(t.transDate, '%d/%m/%Y') dte, sum(PayValue) streamline 
from spendpottenders t where 1=1 and t.PayMethod in (3,4,6,7) and t.transDate [[DDATE]] 
