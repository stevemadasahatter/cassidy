...
$sql=" 
sum(qty) as Qty 
, round(sum(if (od.zero_price=1,0,(if (abs(od.actualgrand) > 0, od.actualgrand*abs(od.qty),od.grandTot*abs(od.qty))))),2) 'Sales'
...
, round((sum(if (od.zero_price=1,0,(if (abs(od.actualnet) > 0, od.actualnet*abs(qty),od.netTot*abs(qty)))))
                - sum(if(abs(od.costprice)>0,abs(od.costprice)*qty,0))),2) as 'Gross Profit'
, round((sum(if (od.zero_price=1,0,(if (abs(od.actualnet) > 0, od.actualnet*abs(qty),od.netTot*abs(qty)))))
                - sum(if(abs(od.costprice)>0,abs(od.costprice)*qty,0)))
		         /sum(if (abs(od.actualnet) > 0, od.actualnet*abs(qty),od.nettot*abs(qty)))*100,2) as 'Margin %'
from orderdetail od, styleDetail style, seasons sea, brands bra, stock sto, category cat, colours col, ProductGroup pg, orderheader oh
where 1=1
...
and od.status not in ( 'A','X','N','P','W')
and [[DATE]] ";
...
$high_titles=array('Info',1,'Sales',3,'',3);
