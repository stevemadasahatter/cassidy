...
, sum(summ.stock) - sum(summ.sales_qty) - sum(summ.adj_qty) - sum(summ.adj_wo) 'C Qty' , sum(summ.instock_cost) 'C Cost'
...
, stockqty.season season_name , stockqty.sea sea, stockqty.bid brand_id , stockqty.stock , stockqty.costprice, soldqty.costofsale, concat(stockqty.sku, ' - ',stockqty.description) skudesc
, stockqty.retailprice , coalesce(soldqty.qty,0) sales_qty, if(abs(stockqty.adj_qty)>0,stockqty.adj_qty,0) adj_qty
, if(abs(stockqty.adj_wo)>0,stockqty.adj_wo,0) adj_wo
...
, (stockqty.costprice * (stockqty.stock - if(abs(stockqty.adj_wo)>0,stockqty.adj_wo,0) - if(abs(soldqty.qty)>0,soldqty.qty,0) - if(abs(stockqty.adj_qty)>0,stockqty.adj_qty,0))) instock_cost
...
, sea.id sid , bra.id bid , bra.nicename brand , adj.wo adj_wo, adj.qty adj_qty, coalesce((physical1 + physical2 + physical3 + physical4 + physical5
+ physical6 + physical7 + physical8 + physical9 + physical10 + physical11 + physical12 + physical13
+ physical14 + physical15+physical16+physical17),0) stock , (purchased1 + purchased2 + purchased3 + purchased4 + purchased5
+ purchased6 + purchased7 + purchased8 + purchased9 + purchased10 + purchased11 + purchased12 + purchased13
+ purchased14 + purchased15+purchased16 + purchased17) purchased from styleDetail sd, brands bra, seasons sea, category cat, stock sto
left join (select sa.sku, sa.colour, sum(if (sa.reasonid <> 5,qty*sr.polarity,0)) qty, sum(if (sa.reasonid = 5,qty*sr.polarity,0)) wo from stkadjustments sa, stkadjreason sr
where sa.reasonid = sr.id and sa.datetime [[DDATE]] group by sku,colour) adj
        on adj.sku = sto.StockRef and adj.colour = sto.colour
where sd.sku = sto.Stockref and bra.id = sd.brand and sea.id = sd.season and cat.id = sd.category 
and (sto.deldate [[DDATE]] or sto.deldate is NULL)) stockqty
left join (select od.StockRef , od.colour , sum(od.qty) qty, sum(od.costprice * od.qty) costofsale
, sum(if (od.zero_price=1,0,(if (abs(od.actualgrand) > 0, od.actualgrand,od.grandTot))*abs(od.qty))) salesvalue
, sum(if (od.zero_price=1,0,(if (abs(od.actualnet) > 0, od.actualnet,od.netTot))*abs(od.qty))) salesnet
, sum(if (od.zero_price=1,0,(if (abs(od.actualvat) > 0, od.actualvat,od.vatTot))*abs(od.qty))) salesvat from orderdetail od
where 1=1 and status not in ( 'A','X','N','P','W')
and [[DATE]]
...
$filters[1]=array('summ.brand_id','summ.season_id','summ.colour','summ.cid', 'summ.skudesc');

#Group by name
$filters[2]=array('summ.brand','summ.sea','summ.colour','summ.category', 'summ.skudesc');

#Pre-select names
$filters[3]=array('summ.brand Brand','summ.sea Season','summ.colour Colour','summ.category Category', 'summ.skudesc SKU');
